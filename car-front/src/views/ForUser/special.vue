<template>
  <div class="special-practice-container">
    <!-- 左右布局容器 -->
    <div class="practice-layout">
      <!-- 左侧：题目展示区域 + 解析区域 -->
      <div class="practice-left">
        <div class="practice-header">
          <h2>{{ subject === 1 ? '科目一' : '科目四' }} {{ model.toUpperCase() }} {{ getQuestionTypeName }} 专项练习</h2>
          <div style="display: flex; align-items: center; gap: 16px;margin-left: 350px">
...
const questionType = ref(0); // 专项题型（1=判断 2=单选 3=多选）
const totalCount = ref(0); // 题目总数
const answerStatus = ref({}); // 答题状态存储（key=题号索引，value=correct/wrong）
const userAnswerRecord = ref({}); // 新增：存储用户每道题答案（用于错题保存，兼容字符串/数组）
const isCurrentSubmitted = ref(false); // 标记当前题目是否已提交答案
const isRecordSaved = ref(false); // 标记：防止重复保存

// ========== 专项练习 关键修改：自定义存储Key ==========
const PRACTICE_PROGRESS_KEY = 'special_practice_progress';
...
const savePracticeProgress = () => {
  const progressData = {
    ...
    userAnswerRecord: userAnswerRecord.value, // 新增：保存用户答案记录（兼容数组）
...
/**
 * 核心新增：专项练习 - 批量保存错题到错题表（兼容多选答案）
 * @param {Number} examRecordId - 关联考试记录ID（兜底非空）
 */
const batchSaveErrorQuestions = async (examRecordId) => {
  try {
    ...
    const validExamRecordId = examRecordId || 1; // 兜底非空
    for (const index of wrongQuestionIndexes) {
      ...
      const errorQuestionData = {
        userId: userId,
        apiId: question.apiId ? String(question.apiId) : (`special_api_${index}`),
    ...
    if (successCount > 0) {
      ElMessage.success(`成功保存${successCount}道错题`);
    }
    if (failCount > 0) {
...
const saveExamRecord = async (needJump = false) => {
  ...
  try {
    ...
    if (!subject.value || !model.value || !questionType.value || totalCount.value === 0) {
...
/**
 * 初始化练习数据（优先从本地存储恢复进度，无则初始化原始数据，兼容多选）
 */
const initPracticeData = () => {
  // ========== 专项练习 关键修改：读取专项练习数据 ==========
  const storageQuestionData = localStorage.getItem('specialPracticeQuestions');
  if (!storageQuestionData) {
    ElMessage.error('未获取到专项练习数据，请返回首页重新开始');
    window.history.back();
    return;
  }
  ...
  questionType.value = parseQuestionData.questionType; // 新增：读取题型
  ...
  if (progressStr) {
    try {
      ...
      if (
          ...
          progressData.questionType === questionType.value &&
        ...
        userAnswerRecord.value = progressData.userAnswerRecord || {}; // 恢复用户答案记录
      }
    } catch (error) {
      console.error('恢复专项练习进度失败：', error);
      localStorage.removeItem(PRACTICE_PROGRESS_KEY);
    }
  }
